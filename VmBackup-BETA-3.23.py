...
# V3.23 June 2018
# NON-PRODUCTION, WORK-In_PROGRESS VERSION -- USE AT YOUR OWN RISK
#
    ...
    for line in config_file:
        if (not line.startswith('#') and len(line.strip()) > 0):
            ...
            if key == 'exclude':
                ...
                #Debug
                ## print "DEBUG: end of save_to_config_exclude"
...
def save_to_config_exclude( key, vm_name):
    ...
    if not found_match:
        ...
        for vm in config[key]:
            ## print "DEBUG: pre-remove vm from list: %s " % vm
            ##print "DEBUG: all vms list: %s " % all_vms
            try:
               all_vms.remove(vm)
            except:
               pass
...
def save_to_config_export( key, value):
    ...
    if not isNormalVmName(vm_name_part) and not isRegExValid( vm_name_part ):
        ...
        return
    for vm in all_vms:
        ## print "DEBUG pre-check for normal VM name: %s " % vm
    ...
    for vdi_parm in config['vdi-export']:
        ...
        for vm_parm in config['vm-export']:
            ...
            if tmp_vm_parm == tmp_vdi_parm:
                log('***WARNING vdi-export dup - removing vm-export=%s' % vm_parm) #debug
